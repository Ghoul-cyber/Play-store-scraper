...
- Google Play has no public bulk-search API. This scrapes the storefront
  ...
  delays are conservative by default; tune `concurrency`/`delayMs` in
  `lib/scraper.js` if you need to go slower.
- Play's DOM/selectors change periodically. Selectors in `lib/scraper.js`
